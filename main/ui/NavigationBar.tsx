...
import { __CHAIN_IDS__, __CHAIN_INFO__, __PROVIDERS__, __STRINGS__ } from "@/constants";
...
function NavigationBar() {
  ...
  const connectWalletModalRef = useRef<HTMLInputElement>(null);
    ...
    <>
      <div className="w-full md:min-h-screen md:border border-[#c6c6c6] flex flex-row md:flex-col justify-between md:justify-start items-start md:items-center px-2 md:px-5 py-2 md:py-5 gap-6">
        ...
        {!isConnected ? (
          <button
            onClick={() => {
              if (connectWalletModalRef.current) connectWalletModalRef.current.checked = true;
            }}
            className="w-full flex justify-start items-center gap-3 bg-[#e7e2b6] capitalize px-2 py-2 rounded-[5px] btn border-none"
          >
            <LuWallet size={27} />
          ...
          <details className="dropdown w-full dropdown-hover">
            <summary className="m-1 btn btn-ghost w-full flex justify-start items-center gap-3 bg-[#e7e2b6] capitalize px-2 py-2 rounded-[5px]">
              <Image
              ...
              />
              <span className="text-sm md:text-lg font-[500] text-[#000]">
                {ellipsize(address as string, 23, {
                  truncate: "middle" as any
                })}
              </span>
            </summary>
            <ul className="p-2 w-full dropdown-content menu rounded-[5px] z-[1] bg-[#e7e2b6] menu-sm shadow-lg border border-[#47473f]">
              <li>
                <a className="flex justify-start items-center gap-3" onClick={() => disconnect({ connector })}>
                  <IoExitOutline size={20} />
                  <span className="capitalize text-sm md:text-lg font-[400] text-[#000]">disconnect wallet</span>
                </a>
              </li>
            </ul>
          </details>
      ...
      <SelectWalletModal
        ...
        close={() => {
          if (connectWalletModalRef.current) connectWalletModalRef.current.checked = false;
        }}
      />
    </>
  );
