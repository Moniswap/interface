"use client";
import { Avatar, Button, Card, SelectCoin, SwapperCoin } from "@/components/common";
  ...
  return (
    <Card data-aos="fade-up" className="w-full xl:w-[60%]">
      <div className="flex gap-10 pb-[44px]  border-b border-black-9">
        ...
        <div>
          <h3 className=" text-[15px]  font-medium leading-[18.15px] text-white">vAMM-WETH/MONI</h3>
      ...
      <SelectCoin
        ...
        value={from}
        resultValueclassName={from ? "border-orange-1 border text-orange-1" : ""}
      />
      <SwapperCoin onClick={swapCoin} className="my-[50px] " />
      <div className="flex justify-between text-white  mb-[29px]">
        <p className="font-medium  text-[15px] leadin-[18px]">Second Token</p>
        <p className="font-medium  text-[13px] leadin-[15px]">Available 0.0 USDC</p>
      </div>

      <SelectCoin value={to} onClick={() => setOpenCointo(true)} />

      <div className="flex mt-[28px]">
        <Button onClick={() => setPathRoute("stoke_liquidity")} theme="dark" className="py-3 rounded-r-none">
          Volatile Pool
        </Button>
        <Button theme="dark-orange" className="rounded-l-none" onClick={() => setPathRoute("stoke_liquidity")}>
          Stable Pool
        </Button>
      </div>

      <CoinModal
        onChange={val => setFrom({ icon: val.icon, label: val.label, value: val.value })}
        open={openCoinFrom}
        close={() => setOpenCoinFrom(false)}
      />
      <CoinModal onChange={val => setTo(val)} open={openCointo} close={() => setOpenCointo(false)} />
    </Card>
  );
};
