...
export const DisconnectModal = ({ open, close }: Props) => {
  ...
  return (
    <Modal hideCloseButton open={open} close={close} className="max-w-[552px] overflow-y-auto h-[80vh]">
            ...
            {TOLERANCE.map(item => {
              return (
                <div
                  ...
                  className={` ${
                    item === procent ? "bg-orange" : ""
                  } py-[11px] h-[34px] xl:h-auto px-[6px] w-[14%]  cursor-pointer flex justify-center items-center border-r border-r-[#403B3B] text-[#CFCFCF] text-[10.056px] xl:text-[13px]  font-bold leading-[normal]`}
                  key={item}
                >
                  <p>{item}</p>
                </div>
      ...
      <div className="border-t mt-[8px] border-t-[#33332D] pt-[13px]">
        <div className="relative rounded-[12.796px]  pb-[20px] bg-[#1E1E1E] pt-[13px] pl-[17px] pr-[15px]">
          <p className="text-[#CFCFCF] text-[13px]  font-semibold leading-[normal]">Allow unsafe trades</p>
          <p className="text-[#7D7D7D] mt-[9px] w-[80%] xl:w-1/2 text-[10.056px] xl:text-[13px]  font-normal leading-[normal]">
            Enabling this will allow trading on quotes with high price impact and could lead to loss of funds.
          </p>
          <div className=" absolute bottom-[12px] right-[12px]">
            <Switch checked={switchDisConnect} onCheckedChange={val => setSwitchDisConnect(val)} />
          </div>
        </div>
      </div>
